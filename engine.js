...
function turnoTipoDe(idx) {
  ...
  if (!t) return 'A';
  return new Date(t).getHours() === 7 ? 'A' : 'B';
}
...
const SUPERVISOR_PALETTE = ['#4C93FF', '#E08A1E', '#1E9E63', '#9B4FD4', '#0F9CB8', '#E0413E', '#B8860B', '#3F9B6E'];
          ...
          <div class="sub-list" style="display:${isOpen ? 'block' : 'none'}">
            ...
            <div class="ot-status-row">
              ...
              <select class="estado-select" data-ot="${ot.otNum}">
                ...
                <option value="Cancelada (en coordinación con sup BHP se canceló)" ${estado==='Cancelada (en coordinación con sup BHP se canceló)'?'selected':''}>Cancelada (en coordinación con sup BHP se canceló)</option>
...
function renderCompRow(c) {
  ...
  const tagColor = { AND: '#9B59D9', ASEO: '#22B37E', INST: '#00B4D8' }[c.tag] || '#9FB4C7';
  return `
...
function renderChart() {
  ...
  svgWrap.innerHTML = `
    <svg viewBox="0 0 ${W} ${H}" preserveAspectRatio="xMidYMid meet" style="width:100%; height:auto; display:block;">
      ...
      ${xAxisTicks}
      ${xAxisLabels}
      ${vis.alcance ? `<path d="${pathFor(alcanceEmerg)}" fill="none" stroke="#2F7BF6" stroke-width="2" stroke-dasharray="5,4"/>` : ''}
      ${vis.plan ? `<path d="${pathFor(percentPlan)}" fill="none" stroke="#2F7BF6" stroke-width="2.5"/>` : ''}
      ...
      ${vis.plan ? dotsFor(percentPlan, '#2F7BF6') : ''}
...
function renderGanttDetail(otNum) {
  ...
  const subs = (ot.subactividades || []).map((s) => {
    ...
    const done = cf >= 0.999;
    const color = done ? 'var(--brand)' : (behind ? 'var(--cancelada)' : '#7BA9F0');
  ...
  wrap.innerHTML = `
    <div class="gantt-detail-header">
      <div class="meta">${ot.pesoPlanHH ? ot.pesoPlanHH.toFixed(1) + ' HH Dimarza · ' : ''}${comps.length ? comps.length + ' de terceros' : 'sin actividades de terceros'}</div>
    </div>
        ...
        if (item.type === 'comp') {
          return `<div class="gantt-detail-item comp">
            ...
            <div class="gdi-info">
              ...
              <div class="gdi-meta">${tagLabel[item.color === '#9B59D9' ? 'AND' : item.color === '#22B37E' ? 'ASEO' : 'INST'] || 'Terceros'} · no cuenta en tu avance</div>
            </div>
          </div>`;
        } else {
...
// --- Clasificacion auxiliar para el protocolo BHP ---
...
async function generatePolinesReportPdf(otNumOrList) {
  ...
  otNums.forEach((otNum, otIdx) => {
    ...
    doc.text('SUPERVISOR BHP ESCONDIDA', marginX + idColW - half + 3, cy + 5);
...
async function generateInformeGeneralPdf() {
  ...
  // ---- Portada ----
  // Logo BHP a la izquierda, logo Dimarza a la derecha — ambos mas grandes y legibles
  ...
  doc.text('SUPERVISOR DMZ', marginX + 3, cy + 5);
  doc.text('SUPERVISOR BHP ESCONDIDA', marginX + half + 11, cy + 5);
  doc.setFont('helvetica', 'normal'); doc.setFontSize(7.5);
